...
// component
const JobsDialog = ({ classes, enqueueSnackbar, open, onClose, t, i18n, role, name }) => {
        ...
        >
            <Mutation 
                ...
                {(createReport, { data, loading, error }) => (
                    <Formik
                        initialValues={{ name: '', date: new Date(), job: '', recruitments: '', terms: '', salary: '' }}
                        validationSchema={() => jobsSchema(t)}
                        onSubmit={async (values, { setSubmitting, resetForm }) => {
                            try {
                                const text = JSON.stringify(values)
                                await createReport({variables: { text: text, type: 'jobs' }})
                                resetForm()
                            } catch(e){
                                setSubmitting(false)
                            }
                        ...
                        >
                            <DialogContent>
                                ...
                                <Typography 
                                    ...
                                    align="center"
                                >
                                    {t('order')} {moment().locale(getLang(i18n)).format('DD MMMM YYYY')}
                                </Typography>
